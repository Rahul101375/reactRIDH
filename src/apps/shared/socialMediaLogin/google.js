...
import {UserProfileComponent} from './profile_info'
...
export default function GoogleLoginComponent() {
    ...
    const onResolve = async ({ provider, data }) => {
      try {
        ...
        const response = await axios.post('https://oauth2.googleapis.com/token', {
          ...
          client_secret: environment?.googleClientSecert, // Replace with your client secret
    ...
    return (
      <>
      {provider && profile && (
          <UserProfileComponent provider={provider} profile={profile} onLogout={onLogout} />
        )}
